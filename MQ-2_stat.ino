 ...
 *  2. курим
    ...
    статистика на скорости 2 не пишется кроме одного случая: если мы дуем-дуем, а показатели всё не падают
...
int ventHiTimeGlobal = 10;      // таймеры
int ventLowTimeGlobal = 300; 
int ventHiTime = ventHiTimeGlobal;
int ventLowTime = ventLowTimeGlobal;
bool lowTimerTick;            
bool hiTimerTick;
...
//  if (currentMillisStat - prevMillisStat >= statTimeInterval) // было
...
//  if (mode == 3 and currentMillisVent - prevMillisVent >= ventTimeInterval) // было
...
//    Serial.println("if sensorStat <= 0");
    ...
    // 0 не накурено
      ...
      } else if (!lowTimerTick and speedd != 1 and ventLowTime != ventLowTimeGlobal) {
        subProg = 1;
        ventLowTime = ventLowTimeGlobal;
      } 
...
//        if (ventTimeInterval > 1000) {
//          ventHiTime = ventHiTime + ventTimeInterval/1000;
//        }
        ventLowTime = ventLowTimeGlobal;
      } 
    } //venttimeInterval
    ...
    *  в этом случае конденсатор первой скорости разрядится на реле */
    if (mode == 3) { // вывод только в режиме "автомат" (mode = 3)
      ...
      // скорость 1
      else if (ventLow == 1 /*and ventHiTime >= 0*/  and (prevVentLow != ventLow or prevVentHi != ventHi)) {
...
//        ventHiTime = ventHiTimeGlobal;
      ...
      if (currentMillis - prevMillis >= printInterval-500) {
        ...
        Serial.print("\ttimers: ");Serial.print(ventLowTime);Serial.print("/");Serial.print(ventHiTime);
      }
      //////////  /пишем в порт ////////// 
